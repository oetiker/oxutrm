...
impl TransportConfig {
    ...
    /// Maximum number of times the probe timeout (PTO) may double while a peer is
    /// unresponsive.
    ///
    /// The PTO backs off exponentially as consecutive probes go unanswered, so the
    /// interval between probes is `pto_base * 2^min(pto_count, this)`. The default of
    /// 16 means a probe interval that can grow to 65536 times the base, which for a
    /// path with a 20ms base is over twenty minutes.
    ///
    /// That is usually harmless, because `max_idle_timeout` closes such a connection
    /// long before the backoff gets that far. **A connection with no idle timeout has
    /// no such bound**, and will keep a peer that has become reachable again waiting
    /// for a probe timer that grew while it was away. Applications that disable the
    /// idle timeout in order to survive long outages -- and that intend to resume the
    /// moment the path returns -- want this low enough that the probe interval stays
    /// within the latency they can accept.
    ///
    /// Lowering it costs probe packets during an outage and nothing at all on a
    /// healthy connection, where `pto_count` is zero.
