...
use oxutrm_proto::{Candidate, ClientSpki, NatType, PathDescription, TermSize, TerminalCaps};
...
use oxutrm_proto::{HostSpki, Signal};
...
async fn serve(detached: oxutrm_host::Detached, root: &RegistryRoot) -> anyhow::Result<()> {
    ...
    let attach = oxutrm_host::begin_attach(&mut meta, HostSpki::new(spki))
    ...
    let permit = oxutrm_host::settle_detachability(&mut meta, path.rung);
